...
def verify_admin_key(request) -> None:
    got = request.headers.get("x-admin-api-key")
    if not got or got != ADMIN_API_KEY:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="invalid admin key")

def decode_token(token: str) -> Dict[str, Any]:
    return jwt.decode(token, SIGNING_KEY, algorithms=[ALGORITHM])

async def get_current_session(request: Request, credentials: Optional[HTTPAuthorizationCredentials]=Depends(security), db: DB=Depends(lambda: request.app.state.db)) -> Dict[str, Any]:
